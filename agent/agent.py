...
class Agents:
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, evaluate=False):
        ...
        # n_action = 5 avail_actions 中装的实际上就是所有可行的action(不管是不是合法的?) yx: 2023-2-28
        ...
        avail_actions = torch.tensor(avail_actions, dtype=torch.float32).unsqueeze(0) # x行的数据, 变成1行x列的数据 yx
        if self.args.cuda:
            inputs = inputs.cuda()
