...
class CRNN(nn.Module):
    def __init__(self, args):
      ...
      # self.bn2 = nn.BatchNorm2d(32)
        ...
        self.rnn = nn.GRUCell(self.size*self.size*32+10, args.rnn_hidden_dim)
        self.fc1 = nn.Linear(args.rnn_hidden_dim, args.n_actions)

    def forward(self, inputs, hidden_state):
        pixel, vec = torch.split(
            inputs, [self.args.fov*self.args.fov*4, self.args.n_agents+self.args.n_actions+2], dim=1)  # FOV尺寸？ 分成FOV*FOV*4, n_agents+n_actions+2
        ...
        pixel = pixel.reshape((-1, self.size*self.size*32))  # (batch,800) ?这个数是啥
