    ...
    def moveOneDroplet(self, droplet_index, action, m_health,
        ...
        if self.distances[i] < goal_dist:  # already achieved goal
            ...
            if new_dist < goal_dist:  # get to the destination
                reward = 0.0
            elif new_dist == self.distances[i] and action == 0:
                reward = -0.2
...
class MEDAEnv(ParallelEnv):
    ...
    def __init__(self, w, l, n_agents, n_blocks=0, fov=19, stall=True, b_degrade=False,
        ...
        assert w > 0 and l > 0
        assert n_agents > 0
        self.mode = 'rgb_array' if show else None
        ...
        self.video = None
        self.color_table = [([255, 0, 0], [255, 153, 153]),  # red
                            ([0, 255, 0], [153, 255, 153]),  # green
                            ([0, 0, 255], [153, 153, 255]),  # blue
                            ([0, 255, 255], [153, 255, 255]),  # 青
                            ([255, 128, 0], [255, 204, 153]),  # orange
                            ([255, 255, 0], [255, 255, 153]),  # yellow
                            ([127, 0, 255], [204, 153, 255]),  # purple
                            ([0, 128, 255], [204, 229, 255]),  # light blue
                            ([255, 0, 255], [255, 153, 255]),  # pink
                            ([255, 0, 127], [255, 204, 299])]  #
        if savemp4:
            self.mode = 'rgb_array'
            file_path = 'video/{}by{}-{}d{}b'.format(
                w, l, n_agents, n_blocks) + str(
                int(time.time())) + ".avi"  # 导出路径

            fourcc = cv2.VideoWriter_fourcc('I', '4', '2', '0')  # 不同视频编码对应不同视频格式（例：'I','4','2','0' 对应avi格式）
            fps = 12
            self.video = cv2.VideoWriter(file_path, fourcc, fps, (l, w))
    ...
    def render(self, close=False):
        ...
        if self.mode == 'human':
            ...
            for drp, des in zip(self.routing_manager.droplets,
                ...
                for y in range(des.y_min, des.y_max + 1):
                    for x in range(des.x_min, des.x_max + 1):
                        img[y][x] = self.color_table[i][1]                  # [0, 255, 0]  # green for destinations
                for y in range(drp.y_min, drp.y_max + 1):
                    for x in range(drp.x_min, drp.x_max + 1):
                        img[y][x] = self.color_table[i][0]   # blue for droplets
                i += 1
...
class MEDAEnv_v0_2(MEDAEnv):
    ...
    def getOneObs(self, agent_index):
        ...
        """
        fov = self.fov
        obs = np.zeros(shape=(4, fov, fov), dtype=np.int8)
        center_x = self.routing_manager.droplets[agent_index].x_center
        center_y = self.routing_manager.droplets[agent_index].y_center
        origin = (center_x - fov // 2, center_y - fov // 2)
        # get droplets in 0 layer
        ...
        if upbound > 0:
            ...
            obs[3, :, -downbound:] = 1
        dir_VEC = np.array([round((des.y_center - center_y)/(self.width/30)), round((des.x_center - center_x)/(self.length/30))], dtype=np.int8)
        obs = np.append(obs, dir_VEC)
        return obs
