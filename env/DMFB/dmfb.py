...
import cv2
'''
DMFBs MARL enviroment created by R.Q. Yang and C. Jiang
'''
...
class Block:
    ...
    def isPointInside(self, point):
        ...
        for i in point:
            if self.x_min <= i[0] <= self.x_max and \
                    self.y_min <= i[1] <= self.y_max:
                return True
        else:
            return False
...
class RoutingTaskManager:
    def __init__(self, width, length, n_droplets,
                 n_blocks=0, fov=5, stall=True, b_degrade=True, per_degrade=0.1, isTrain=True, heal_init=1.0):
        self.width = width
        self.length = length
        self.n_droplets = n_droplets
        self.n_blocks = n_blocks
        self.droplets = []
        self.heal_init = heal_init
    ...
    def refresh(self, new=False):
        ...
        self.Generate_task()
        if self.isTrain:
            # self.m_health = np.random.uniform(self.heal_init, min(1.0, self.heal_init + 0.2), (self.width, self.length))
    ...
    def moveDroplets(self, actions):
        ...
        # yx: constraints 约束，分为静态约束和动态的约束
        rewards = np.array(rewards)-2*sta-2*dy
        if self.stall:
            for i in range(self.n_droplets):
                if dones[i]:
                    rewards[i] = 0
        # yx: 如果task完成了，获得一定的reward；同时，如果期间没有冲突，获得一定的reward
    ...
    def _isTouchingBlocks(self, point):
        for m in self.blocks:
            if m.x_min <= point[0] <= m.x_max and \
                    m.y_min <= point[1] <= m.y_max:
                return True
        return False
    ...
    def moveOneDroplet(self, droplet_index, action):
        ...
        if self.stall and self.distances[droplet_index] == 0:
            ...
            if new_dist == self.distances[droplet_index] and self.distances[droplet_index] == 0:
                reward = 0  # the origin version is different from the paper.
            # stall in past position
            elif new_dist == self.distances[droplet_index] and action == 0:
                reward = -0.25
            # closer to the destination
            elif new_dist < self.distances[droplet_index]:
                reward = -0.1
            # penalty for getting far away from the target
            else:
                reward = -0.4
            prev_health = self.m_health[x, y]
            new_health = self.m_health[self.droplets[droplet_index].x, self.droplets[droplet_index].y]
            if prev_health > new_health:
                # 移动到健康程度更差的地方
                reward -= np.exp(prev_health - new_health)
            elif prev_health < new_health:
                reward += new_health - prev_health
                # 移动到健康程度更好的地方
    ...
    def getglobalobs(self):
        ...
        // Health(replace obstacles?) in layer 3(2) [0 0 x]
        '''
        def add_health_In_global_Obs(g_obs):
            # TODO: yx 2023/03/09 add the health information on the board for the input.
            pass
...
class DMFBenv(ParallelEnv):
    ...
    def __init__(self, width, length, n_agents, n_blocks=0, fov=5, stall=True, b_degrade=False, per_degrade=0.1, show=False, savemp4=False, isTrain=True, heal_init=1):
        ...
        self.observation_spaces = {name: spaces.Box(
            low=0, high=n_agents, shape=(3, width, length), dtype=np.uint8)
            for name in self.agents}
        ...
        self.routing_manager = RoutingTaskManager(
            width, length, n_agents, n_blocks, fov, stall, b_degrade, per_degrade, isTrain, heal_init)
...
# used for qmix
    ...
    def render(self, close=False):
        ...
        screen_length = self.length*u_size #self.screen_length
        screen_width = self.width*u_size#self.screen_width
    ...
    # blocks
        if self.routing_manager.n_blocks > 0:
            pass
...
# adding 5.1
class DMFBenv_v0_2(DMFBenv):
    def __init__(self, w, l, n, **kwargs):
        kwargs['b_degrade'] = True
        super().__init__(w, l, n, **kwargs)
    ...
    def getOneObs(self, agent_i):
        ...
        current droplet's goal in layer 1  [x id x 0]
        other's Goal           in layer 2  [x x id 0]
        Health state           in layer 3  [x 0  0 1]
        '''
        fov = self.routing_manager.fov  # 正方形fov边长
        obs_i = np.zeros((4, fov, fov))
        droplets = self.routing_manager.droplets
        ...
        # get blocks layer 3
        # add boundary
        left_bound = fov // 2 - center_x
        right_bound = fov // 2 - (self.width - 1 - center_x)
        up_bound = fov // 2 - center_y
        down_bound = fov // 2 - (self.length - 1 - center_y)

        if left_bound <= 0 and right_bound <= 0 and up_bound <= 0 and down_bound <= 0:
            pass
            obs_i[3, :, :] = self.routing_manager.m_health[-left_bound:-left_bound + fov, -up_bound: -up_bound + fov]
        else:
            for i in range(0, fov):
                for j in range(0, fov):
                    p, q = center_x - fov // 2 + i, center_y - fov // 2 + j
                    if 0 <= p < self.width and 0 <= q < self.length:
                        obs_i[3, i, j] = self.routing_manager.m_health[p, q]
                    else:
                        obs_i[3, i, j] = 0
        for block in self.routing_manager.blocks:
            for i in range(block.x_min, block.x_max+1):
